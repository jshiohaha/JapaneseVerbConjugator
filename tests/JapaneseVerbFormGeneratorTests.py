...
class TestPositiveVerbForms(ParametrizedTestCase):
    ...
    '''
    def setUp(self):
        self.positiveVerbFormGenerator = PositiveVerbForms()
        self.verb = self.param
    
    # Polite Verb Forms Test
    def test_polite_positive_nonpast(self):
        result = self.positiveVerbFormGenerator.generate_polite_form(self.verb.Verb, self.verb.Verb_Class, Tense.NONPAST)
        self.assertEqual(result, self.verb.PolitePositiveNonpast)

    def test_polite_positive_past(self):
        result = self.positiveVerbFormGenerator.generate_polite_form(self.verb.Verb, self.verb.Verb_Class, Tense.PAST)
        self.assertEqual(result, self.verb.PolitePositivePast)

    # Plain Verb Forms Test
    def test_plain_positive_nonpast(self):
        result = self.positiveVerbFormGenerator.generate_plain_form(self.verb.Verb, self.verb.Verb_Class, Tense.NONPAST)
        self.assertEqual(result, self.verb.Verb)

    def test_plain_positive_past (self):
        result = self.positiveVerbFormGenerator.generate_plain_form(self.verb.Verb, self.verb.Verb_Class, Tense.PAST)
        self.assertEqual(result, self.verb.PlainPositivePast)

    # Te Verb Form Test
    def test_te_form(self):
        result = self.positiveVerbFormGenerator.generate_te_form(self.verb.Verb, self.verb.Verb_Class)
    ...
    def test_conditional_plain(self):
        ...
        result = self.positiveVerbFormGenerator.generate_conditional_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.ConditionalPlain)

    def test_conditional_polite(self):
        if self.verb.Verb_Class == VerbClass.IRREGULAR:
            self.skipTest("Not Required for Irregular Verbs")
        result = self.positiveVerbFormGenerator.generate_conditional_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_volitional_plain_positive(self):
        # if self.verb.Verb_Class == VerbClass.IRREGULAR:
        #     self.skipTest("Not Required for Irregular Verbs")
        result = self.positiveVerbFormGenerator.generate_volitional_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.VolitionalPlainPositive)

    def test_volitional_polite_positive(self):
        if self.verb.Verb_Class == VerbClass.IRREGULAR:
            self.skipTest("Not Required for Irregular Verbs")
        result = self.positiveVerbFormGenerator.generate_volitional_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_potential_plain_positive(self):
        result = self.positiveVerbFormGenerator.generate_potential_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.PotentialPlainPositive)

    def test_potential_polite_positive(self):
        result = self.positiveVerbFormGenerator.generate_potential_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_imperative_plain_positive(self):
        result = self.positiveVerbFormGenerator.generate_imperative_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.ImperativePlainPositive)

    def test_imperative_polite_positive(self):
        result = self.positiveVerbFormGenerator.generate_imperative_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_provisional_plain_positive(self):
        result = self.positiveVerbFormGenerator.generate_provisional_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
    ...
    def test_provisional_polite_positive(self):
        ...
        result = self.positiveVerbFormGenerator.generate_provisional_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_causative_plain_positive(self):
        result = self.positiveVerbFormGenerator.generate_causative_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.CausativePlainPositive)

    def test_causative_polite_positive(self):
        if self.verb.Verb_Class == VerbClass.IRREGULAR:
            self.skipTest("Not Required for Irregular Verbs")
        result = self.positiveVerbFormGenerator.generate_causative_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
        self.assertEqual(result, self.verb.CausativePolitePositive)

    # Passive Verb Forms Test
    def test_passive_plain_positive(self):
        result = self.positiveVerbFormGenerator.generate_passive_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.PassivePlainPositive)

    def test_passive_polite_positive(self):
        if self.verb.Verb_Class == VerbClass.IRREGULAR:
            self.skipTest("Not Required for Irregular Verbs")
        result = self.positiveVerbFormGenerator.generate_passive_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
...
class TestNegativeVerbForms(ParametrizedTestCase):
    ...
    '''
    def setUp(self):
        self.negativeVerbFormGenerator = NegativeVerbForms()
        self.verb = self.param

    # Formal Verb Forms Test
    def test_polite_negative_nonpast(self):
        result = self.negativeVerbFormGenerator.generate_polite_form(self.verb.Verb, self.verb.Verb_Class, Tense.NONPAST)
        self.assertEqual(result, self.verb.PoliteNegativeNonpast)

    def test_polite_negative_past(self):
        result = self.negativeVerbFormGenerator.generate_polite_form(self.verb.Verb, self.verb.Verb_Class, Tense.PAST)
        self.assertEqual(result, self.verb.PoliteNegativePast)

    # Plain Verb Forms Test
    def test_plain_negative_nonpast (self):
        result = self.negativeVerbFormGenerator.generate_plain_form(self.verb.Verb, self.verb.Verb_Class, Tense.NONPAST)
        self.assertEqual(result, self.verb.PlainNegativeNonpast)

    def test_plain_negative_past (self):
        result = self.negativeVerbFormGenerator.generate_plain_form(self.verb.Verb, self.verb.Verb_Class, Tense.PAST)
    ...
    def test_conditional_plain(self):
        if self.verb.Verb_Class is not VerbClass.IRREGULAR:
            self.skipTest("Not Required for Non-Irregular Verbs")
        result = self.negativeVerbFormGenerator.generate_conditional_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.ConditionalPlainNegative)

    def test_conditional_polite(self):
        if self.verb.Verb_Class is not VerbClass.IRREGULAR:
            self.skipTest("Not Required for Non-Irregular Verbs")
        result = self.negativeVerbFormGenerator.generate_conditional_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
        self.assertEqual(result, self.verb.ConditionalPoliteNegative)

    # Volitional Verb Forms Test
    def test_volitional_plain_negative(self):
        result = self.negativeVerbFormGenerator.generate_volitional_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.VolitionalPlainNegative)

    def test_volitional_polite_negative(self):
        result = self.negativeVerbFormGenerator.generate_volitional_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
        self.assertEqual(result, self.verb.VolitionalPoliteNegative)

    # Potential Verb Forms Test
    def test_potential_plain_negative(self):
        result = self.negativeVerbFormGenerator.generate_potential_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.PotentialPlainNegative)

    def test_potential_polite_negative(self):
        result = self.negativeVerbFormGenerator.generate_potential_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
        self.assertEqual(result, self.verb.PotentialPoliteNegative)

    # Imperative Verb Forms Test
    def test_imperative_plain_negative(self):
        result = self.negativeVerbFormGenerator.generate_imperative_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.ImperativePlainNegative)

    def test_imperative_polite_negative(self):
        result = self.negativeVerbFormGenerator.generate_imperative_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
        self.assertEqual(result, self.verb.ImperativePoliteNegative)

    # Provisional Verb Forms Test
    def test_provisional_plain_negative(self):
        result = self.negativeVerbFormGenerator.generate_provisional_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.ProvisionalPlainNegative)

    def test_provisional_polite_negative(self):
        if self.verb.Verb_Class is not VerbClass.IRREGULAR:
            self.skipTest("Not required for non-irregular verbs")
        result = self.negativeVerbFormGenerator.generate_provisional_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_causative_plain_negative(self):
        ...
        result = self.negativeVerbFormGenerator.generate_causative_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.CausativePlainNegative)

    def test_causative_polite_negative(self):
        if self.verb.Verb[-2:] == "する":
            self.skipTest("Not required for する verbs")
        result = self.negativeVerbFormGenerator.generate_causative_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
    ...
    def test_passive_plain_negative(self):
        if self.verb.Verb_Class == VerbClass.IRREGULAR:
            self.skipTest("Not required for irregular verbs")
        result = self.negativeVerbFormGenerator.generate_passive_form(self.verb.Verb, self.verb.Verb_Class, Formality.PLAIN)
        self.assertEqual(result, self.verb.PassivePlainNegative)

    def test_passive_polite_negative(self):
        if self.verb.Verb_Class == VerbClass.IRREGULAR:
            self.skipTest("Not required for irregular verbs")
        result = self.negativeVerbFormGenerator.generate_passive_form(self.verb.Verb, self.verb.Verb_Class, Formality.POLITE)
...
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite = create_positive_verb_form_suite(suite)
    suite = create_negative_verb_form_suite(suite)
